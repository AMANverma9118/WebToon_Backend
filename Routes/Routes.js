...
router.get("/fetchallwebtoons", async (req, res) => {
  try {
    const webtoons = await Webtoon.find({}).select(
      "title description characters imageUrl createdAt",
    );
    res.json(webtoons);
  } catch (error) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!creatorId) {
        return res
          .status(400)
          .json({ error: "Creator ID is missing in the token." });
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (webtoon.createdBy.toString() !== req.user.id) {
        return res
          .status(403)
          .json({ error: "User not authorized to update this webtoon" });
      }
      ...
      webtoon = await Webtoon.findByIdAndUpdate(
        req.params.id,
        { $set: updatedWebtoon },
        { new: true },
      );
...
router.delete("/deletewebtoon/:id", fetchuser, async (req, res) => {
  try {
    ...
    if (webtoon.createdBy.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ error: "User not authorized to delete this webtoon" });
    }
